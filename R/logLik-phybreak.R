...
logLik.phybreak <- function(object, genetic = TRUE, withinhost = TRUE, sampling = TRUE, generation = TRUE, ...) {
    ...
    for(gene in 1:object$d$ngenes) {
      res <- res + with(object, 
                        .likseq(matrix(unlist(d$sequences[[gene]]), ncol = p$obs),
                                ...
                                v$nodeparents[gene, ], v$nodetimes[gene, ],
                                p$mu, p$obs))
    }
  }
  
  if(generation) {
    res <- res + with(object, .lik.gentimes(p$obs, p$shape.gen, p$mean.gen, v$nodetimes[1, ], v$nodehosts, v$nodetypes)) 
  }
  
  if(sampling) {
    res <- res + with(object, .lik.sampletimes(p$shape.sample, p$mean.sample, v$nodetimes[1, ], v$nodetypes))   
  }
  ...
  attributes(res) <- list(
    nobs = object$p$obs,
    df = 1 + object$h$est.mG + object$h$est.mS + object$h$est.wh + object$h$est.reass,
