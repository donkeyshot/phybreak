...
### posterior trees (0 = current) called by: plot.phybreak calls: phybreak2phylo

#' @describeIn get.phybreak Returns an object of class \code{\link[ape]{phylo}} and optionally of class 
...
### phylogenetic trees in class 'multiPhylo'. The function also allows to get one of the
### posterior trees (0 = current) called by: plot.phybreak calls: .makephylosimmap.phybreak .makephylo.phybreak

#' @describeIn get.phybreak Returns an object of class \code{\link[ape]{multiphylo}} and optionally of class 
#'   \code{"simmap"} (package \pkg{phytools}).
#'   
#' @param thin Thinning interval for making the \code{\link[ape]{multiphylo}}-object.
...
#'        sampling = FALSE, generation = FALSE) #should give the same result as 'pml'
#' @export
get.multiPhylo <- function(phybreak.object, thin = 1, nkeep = Inf, simmap = FALSE) {
  ### tests
  if (simmap & !("phytools" %in% .packages(TRUE))) {
    warning("package 'phytools' is not installed; changed input to simmap = FALSE")
  }
  ...
  ### make trees
  res <- lapply(tokeep, get.phylo, phybreak.object = phybreak.object, simmap = simmap)
  names(res) <- tokeep
