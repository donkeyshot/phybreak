...
# 
#   # bottleneck nodes
#   if(bottleneckmodel <- pbe1$p$wh.model %in% c(4, 5, "exponential", "constant")) {
...
#   rewire_pullnodes(pbe1$v$infectors[pbe1$hostID])
# }
# 
# rewire_pathD <- function() {
#   ### First, dismantle minitree
#   # edges entering hostID, with endtimes
#   edgesin <- which(pbe1$v$nodehosts == pbe1$hostID & pbe1$v$nodetypes %in% c("s", "x", "t", "b"))
#   edgeintimes <- pbe1$v$nodetimes[edgesin]
...
#   if(bottleneckmodel) coalnodes <- c(which(pbe1$v$nodehosts == 0 & pbe1$v$nodetypes == "c"), coalnodes)
#   
...
#   rewire_pullnodes(pbe1$hostID)
#   rewire_pullnodes(newinfector)
#   # 
...
# 
# rewire_pullnodes <- function(currentID) {
...
#         pbe1$v$nodeparents[childnodes] <- parentnodes
#         
#         rewire_pullnodes(pbe1$v$infectors[currentID])
#         
#       }
#     }
#   }
