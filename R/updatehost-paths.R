### all helper functions exclusively involved in updating the tree, ###
### i.e. proposing a new tree and accepting or rejecting the proposal ###
# fixed parameters
tinf.prop.shape.mult <- 2/3 #shape for proposing infection time is shape.sample * tinf.prop.shape.mult

### updating both transmission and phylotree:
### proposing an infection time and following the decision tree
### called from:
# burnin.phybreak
# sample.phybreak
### calling:
# .prepare.phybreakenv
# .updatepathA, .updatepathB, .updatepathC1, .updatepathC2,
...
.updatehost <- function(hostID) {
  ...
  evalq({
    ...
    tinf.prop <- v$nodetimes[hostID] - 
      rgamma(1,shape=tinf.prop.shape.mult * p$shape.sample,
    ...
    if(v$nodehosts[hostID + 2*p$obs - 1] == 0) {
      ...
      if(tinf.prop < min(c(v$nodetimes[v$nodetypes == "t" & v$nodehosts == hostID], Inf))) {
        ...
        if(tinf.prop < sort(c(v$nodetimes[v$nodetypes == "t" & v$nodehosts == hostID], Inf))[2]) {
          ...
          if(runif(1) < 0.5) .updatepathC(TRUE) else .updatepathC(FALSE)
        }
      }
    } else {
      ...
      if(tinf.prop < v$nodetimes[v$nodehosts == 0]) {
        ...
        if(tinf.prop < min(c(v$nodetimes[v$nodetypes == "t" & v$nodehosts == hostID], Inf))) {
          ...
          if(runif(1) < 0.5) .updatepathF(TRUE) else .updatepathF(FALSE)
        }
      }
    }
  }, .phybreakenv.prop)
...
### updating transmission tree but keeping phylotree:
### proposing an infection time and following the decision tree
### called from:
# burnin.phybreak
# sample.phybreak
...
# .prepare.phybreakenv
# .ptr  ##C++
# .updatepathG, .updatepathH, .updatepathI, .updatepathJ
...
{
### update if hostID is index and tinf.prop is before the
...
### update if hostID is index and tinf.prop is after the
### first secondary case, but before the second secondary case
### called from:
# .updatehost
### calling:
# .samplecoaltimes
...
.updatepathB <- function() {
  ...
  evalq({
    ...
    # prepare for phylotree proposals by moving the coalescent node 
    # from hostID to the new infector
...
.updatepathC <- function(exchange) {
  ...
  evalq({
    ...
    # prepare for phylotree proposals by moving coalescent nodes 
    # between hostID and infectee.current.ID
    if(exchange) {
    ...
    if(infector.current.ID != 0) {
      ...
      # second infectee not necessarily the same for reversal proposal, 
      # so identify these intervals for hostID and new index
...
### update if hostID is not index and tinf.prop is before 
### infection of the index case
### called from:
# .updatehost
### calling:
# .samplecoaltimes
...
.updatepathD <- function() {
  ...
  evalq({
    ...
    # prepare for phylotree proposals by moving coalescent nodes 
    # from current infector to hostID
...
### update if hostID is not index and tinf.prop is after infection
### of the index case, but before the first secondary case
### called from:
# .updatehost
### calling:
# .samplecoaltimes
...
.updatepathE <- function() {
  ...
  evalq({
    ...
    # prepare for phylotree proposals by moving the coalescent node 
    # from the current infector to the proposed infector
...
### update if hostID is not index and tinf.prop is after the
### first secondary case
### called from:
# .updatehost
### calling:
...
.updatepathH <- function() {
  evalq({
    ...
    # bookkeeping: move the transmission node in the phylotree
    # first, remove it by connecting the upstream and downstream nodes
    ...
    # bookkeeping: give all infectees of hostID and hostiorID
    # their correct infector according to the proposed transmission node
...
### update if hostID is not index, tinf.prop is before the
### MRCA of hostID and infector, and infector is index
### called from:
# .updatehost.keepphylo
### calling:
# .ptr  ##C++
...
### update if hostID is not index, tinf.prop is before the
### MRCA of hostID and infector, infector is not index,
### and tinf.prop is after the MRCA of hostID and infector's infector
### called from:
# .updatehost.keepphylo
### calling:
# .ptr  ##C++
