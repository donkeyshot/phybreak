...
### burnin.phybreak sample.phybreak calling: .prepare.phybreakenv .updatepathA, .updatepathB, .updatepathC1, .updatepathC2,
...
.updatehost <- function(hostID) {
    ...
    evalq({
        ...
        tinf.prop <- v$nodetimes[hostID] - rgamma(1, shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
            p$shape.sample))
...
### burnin.phybreak sample.phybreak calling: .prepare.phybreakenv .ptr ##C++ .updatepathG, .updatepathH, .updatepathI,
### .updatepathJ
    ...
    evalq({
        ...
        if (hostiorID == 0) {
            ...
            if (tinf.prop > timemrca) {
                ...
                tinf2.prop <- v$nodetimes[hostiorID] - rgamma(1, shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
                  p$shape.sample))
...

{
    ...
    ### .samplecoaltimes .sampletopology .propose.phybreakenv .accept.phybreakenv
    .updatepathA <- function() {
        ...
        evalq({
            ...
            # phylotree in hostID
            v$nodetimes[v$nodehosts == hostID & v$nodetypes == "c"] <- v$nodetimes[hostID + 2 * p$obs - 1] + .samplecoaltimes(v$nodetimes[v$nodehosts == 
                hostID & v$nodetypes != "c"] - v$nodetimes[hostID + 2 * p$obs - 1], p$wh.model, p$wh.slope)
            v$nodeparents[v$nodehosts == hostID] <- .sampletopology(which(v$nodehosts == hostID), v$nodetimes[v$nodehosts == 
                hostID], v$nodetypes[v$nodehosts == hostID], hostID + 2 * p$obs - 1, p$wh.model)
            
            
            ### calculate proposal ratio
            logproposalratio <- dgamma(.phybreakenv$v$nodetimes[hostID] - .phybreakenv$v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * 
                p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample), log = TRUE) - dgamma(v$nodetimes[hostID] - 
                v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
                p$shape.sample), log = TRUE)
            
        }, .phybreakenv.prop)
        
        
        ### calculate likelihood
        .propose.phybreakenv("phylotrans")
        
        ### calculate acceptance probability
        evalq(logaccprob <- logLik + logLikgen + logLiksam - .phybreakenv$logLik - .phybreakenv$logLikgen - .phybreakenv$logLiksam + 
            logproposalratio, .phybreakenv.prop)
    ...
    ### from: .updatehost calling: .samplecoaltimes .sampletopology .propose.phybreakenv .accept.phybreakenv
    .updatepathB <- function() {
        ...
        evalq({
            ...
            dens.infectorproposal <- (dgamma(tinf.prop - tail(v$nodetimes, p$obs), shape = p$shape.gen, scale = p$mean.gen/p$shape.gen)) + 
            ...
            for (ID in c(hostID, infector.proposed.ID)) {
                v$nodetimes[v$nodehosts == ID & v$nodetypes == "c"] <- v$nodetimes[ID + 2 * p$obs - 1] + .samplecoaltimes(v$nodetimes[v$nodehosts == 
                  ID & v$nodetypes != "c"] - v$nodetimes[ID + 2 * p$obs - 1], p$wh.model, p$wh.slope)
            ...
            ### calculate proposal ratio
            logproposalratio <- log(sum(dens.infectorproposal)/(dens.infectorproposal[infector.proposed.ID])) + dgamma(.phybreakenv$v$nodetimes[hostID] - 
                .phybreakenv$v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
                p$shape.sample), log = TRUE) - dgamma(v$nodetimes[hostID] - v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * 
                p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample), log = TRUE)
    ...
    .updatepathC <- function(exchange) {
        .phybreakenv.prop$exchange <- exchange
        ...
        evalq({
            ...
            if (infector.current.ID != 0) {
                ...
                logproposalratio <- pgamma(v$nodetimes[infectee.current.ID] - v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * 
                  p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample), log.p = TRUE) - pgamma(v$nodetimes[hostID] - 
                  v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
                  p$shape.sample), log.p = TRUE)
            } else {
                ...
                logproposalratio <- pgamma(sampleinterval.newindex, shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
                  p$shape.sample), log.p = TRUE) - pgamma(sampleinterval.current.ID, shape = tinf.prop.shape.mult * p$shape.sample, 
        ...
        ### calculate acceptance probability
        evalq({
            logaccprob <- logLik + logLikgen + logLiksam - .phybreakenv$logLik - .phybreakenv$logLikgen - .phybreakenv$logLiksam + 
                logproposalratio
        }, .phybreakenv.prop)
    ...
    .updatepathD <- function() {
        ...
        evalq({
            ...
            dens.infectorcurrent <- (dgamma(.phybreakenv$v$nodetimes[hostID + 2 * p$obs - 1] - tail(.phybreakenv$v$nodetimes, 
                p$obs), shape = p$shape.gen, scale = p$mean.gen/p$shape.gen)) + (.phybreakenv$v$nodetimes[hostID + 2 * p$obs - 
                1] - tail(.phybreakenv$v$nodetimes, p$obs) > 0)/h$dist[hostID, ]
            dens.infectorcurrent[hostID] <- 0
            
            logproposalratio <- log(dens.infectorcurrent[infector.current.ID]/(sum(dens.infectorcurrent))) + dgamma(.phybreakenv$v$nodetimes[hostID] - 
                .phybreakenv$v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
                p$shape.sample), log = TRUE) - dgamma(v$nodetimes[hostID] - v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * 
                p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample), log = TRUE)
        }, .phybreakenv.prop)
    ...
    ### called from: .updatehost calling: .samplecoaltimes .sampletopology .propose.phybreakenv .accept.phybreakenv
    .updatepathE <- function() {
        ...
        evalq({
            ...
            logproposalratio <- log(dens.infectorcurrent[infector.current.ID] * sum(dens.infectorproposal)/(dens.infectorproposal[infector.proposed.ID] * 
                sum(dens.infectorcurrent))) + dgamma(.phybreakenv$v$nodetimes[hostID] - .phybreakenv$v$nodetimes[hostID + 2 * 
                p$obs - 1], shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample), 
                log = TRUE) - dgamma(v$nodetimes[hostID] - v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * 
                p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample), log = TRUE)
        }, .phybreakenv.prop)
...

{
    ...
    ### .propose.phybreakenv .accept.phybreakenv
    .updatepathG <- function() {
        ...
        ### calculate likelihood
        .propose.phybreakenv("trans")
        
        ### calculate acceptance probability
        evalq(logaccprob <- logLikgen + logLiksam + logLikcoal - .phybreakenv$logLikgen - .phybreakenv$logLiksam - .phybreakenv$logLikcoal + 
            logproposalratio, .phybreakenv.prop)
    ...
    ### calling: .ptr ##C++ .propose.phybreakenv .accept.phybreakenv
    ...
    ### .updatehost.keepphylo calling: .ptr ##C++ .propose.phybreakenv .accept.phybreakenv
    .updatepathI <- function() {
        evalq({
            ...
            ### calculate proposal ratio
            logproposalratio <- dgamma(.phybreakenv$v$nodetimes[hostID] - .phybreakenv$v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * 
                p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample), log = TRUE) - log(1 - pgamma(v$nodetimes[hostID] - 
                timemrca, shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample))) + 
                log(1 - pgamma(.phybreakenv$v$nodetimes[hostiorID] - timemrca, shape = tinf.prop.shape.mult * p$shape.sample, 
                  scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample))) - dgamma(v$nodetimes[hostiorID] - v$nodetimes[hostiorID + 
                2 * p$obs - 1], shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
                p$shape.sample), log = TRUE)
        }, .phybreakenv.prop)
    ...
    .updatepathJ <- function() {
        evalq({
            ...
            logproposalratio <- dgamma(.phybreakenv$v$nodetimes[hostID] - .phybreakenv$v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * 
                p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample), log = TRUE) - dgamma(v$nodetimes[hostID] - 
                v$nodetimes[hostID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
                p$shape.sample), log = TRUE) + dgamma(.phybreakenv$v$nodetimes[hostiorID] - .phybreakenv$v$nodetimes[hostiorID + 
                2 * p$obs - 1], shape = tinf.prop.shape.mult * p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * 
                p$shape.sample), log = TRUE) - dgamma(v$nodetimes[hostiorID] - v$nodetimes[hostiorID + 2 * p$obs - 1], shape = tinf.prop.shape.mult * 
                p$shape.sample, scale = p$mean.sample/(tinf.prop.shape.mult * p$shape.sample), log = TRUE)
        }, .phybreakenv.prop)
