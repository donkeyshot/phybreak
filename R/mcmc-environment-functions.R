#The environments .phybreakenv and .phybreakenv.prop will contain 
#phybreak objects (excluding mcmc samples) + likelihood calculations. 
#.phybreakenv is consistent with the current state of updating 
#phybreak.object; .phybreakenv.prop
#is used to propose states and calculate likelihoods for these proposals.
...
#and sampling times with means mG and mS. logLikcoal is the log-likelihood
#value of the coalescent model. 

#The function '.build.phybreakenv' is used to initialise
#The function '.destroy.phybreakenv' is used at the end
#The function '.prepare.phybreakenv' is used to prepare .phybreakenv.prop 
#  for another proposal
#The function '.propose.phybreakenv' is used to calculate the likelihoods
#  in .phybreakenv.prop after changing in proposal functions. 
#The function '.accept.phybreakenv' is to change .phybreakenv 
#  by copying .phybreakenv.prop.
...
### build the environments at the start of an mcmc chain by
### copying the fixed parameters and phybreak object,
### and by calculating likarray and the log-likelihoods
### called from:
# burnin.phybreak
# sample.phybreak
### calls:
# .likseqenv  ## C++ function
# .liksampletimes
# .likgentimes
# .likcoaltimes
# .accept.phybreakenv
.build.phybreakenv <- function(phybreak.obj) {
  ...
  ###Jukes-Cantor: reduce diversity by naming each nucleotide by its
  ###frequency order, and grouping SNPs by same pattern across hosts
  ...
  .phybreakenv.prop$likarray <- array(1, dim = c(4,length(snpfrreduced),
  ...
  .phybreakenv.prop$likarray[cbind(1,
                                   rep(1:length(snpfrreduced),
                                       .phybreakenv.prop$p$obs),
                                   rep(1:.phybreakenv.prop$p$obs,
                                       each = length(snpfrreduced))
  )
  ] <- 1 * t(snpreduced == 1 | snpreduced == 5)
  ...
  .phybreakenv.prop$logLikcoal <- with(phybreak.obj,
                                       .lik.coaltimes(p$obs, p$wh.model, p$wh.slope, 
...
### take the elements d, v, p, and h from .phybreakenv,
### and s from the function arguments, and make a new
### phybreak-object. Then empty the environments and return
### the new object.
### called from:
# burnin.phybreak
# sample.phybreak
...
### copy the elements from .phybreakenv into .phybreakenv.prop
### to prepare for a proposal
### called from:
# .updatehost
# .update.mu, .update.mS, .update.mG, .update.wh
...
### calculate the new log-likelihoods where necessary
### and adjust likarray. Argument f indicates which type of
### function it is called from
### called from:
# .updatepathA - .updatepathJ
# .update.mu, .update.mS, .update.mG, .update.wh
### calls:
# .likseqenv  ## C++ function
# .liksampletimes
...
.propose.phybreakenv <- function(f) {
  ...
  if(f == "phylotrans" || f == "trans" || f == "mS") {
    evalq( {
      logLiksam <- .lik.sampletimes(
        p$shape.sample, p$mean.sample,
  ...
  if(f == "phylotrans" || f == "trans" || f == "slope") {
    evalq( {
      logLikcoal <- .lik.coaltimes(
        p$obs, p$wh.model, 
        p$wh.slope, v$nodetimes, 
        v$nodehosts, v$nodetypes)
        },
          envir = .phybreakenv.prop)
  } 
...
### copy the elements from .phybreakenv.prop into .phybreakenv
### upon acceptance of a proposal
### called from:
# .updatepathA - .updatepathJ
# .update.mu, .update.mS, .update.mG, .update.wh
