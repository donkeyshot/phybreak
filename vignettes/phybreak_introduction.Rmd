...
vignette: >
  %\VignetteIndexEntry{Introduction to \code{phybreak} package}
...
```{r, echo =FALSE}
knitr::opts_chunk$set(collapse = TRUE, comment = "#>", size = "LARGE")
```
...
names_FMD2001 <- sapply(namesdates_FMD2001, '[', i = 1)               #first element is name
dates_FMD2001 <- as.numeric(sapply(namesdates_FMD2001, '[', i = 2))   #second element is date
...
```{r burnin, cache=TRUE, cache.lazy=FALSE, autodep=TRUE}
...
set.seed(0)
```
```{r FMDmultiple, autodep=TRUE, cache=TRUE, cache.lazy=FALSE, include=FALSE}
