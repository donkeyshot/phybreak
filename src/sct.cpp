...
long double sum(std::vector<long double> x) {
  int l = x.size();
  long double v = 0;
  for(int i = 0; i < l; ++i) {
    v += x[i];
...
// [[Rcpp::export(name=".sct")]]
NumericVector sct(NumericVector tle) { //in reverse order
  int nleafs = tle.size();
  ...
  randnr = -log(runif(1,exp(-tle[1]),1)[0]);
  ...
  for(int i = 2; i < nleafs; ++i) {
    ...
    randnr = -log(runif(1,exp(-sum(cumrates)),1)[0]);
